            ...
            <div x-data="{ showFilters: false }" class="bg-white p-4 border-b border-gray-200 bg-gray-50 mb-0">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div class="w-full sm:max-w-xs relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari judul galeri..." class="bg-gray-100 focus:bg-white text-sm border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm block w-full pl-9 py-3 transition-colors">
                    </div>
                    ...
                    <div class="pt-4 mt-4 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <select wire:model.live="filterMonth" class="w-1/2 bg-white text-sm border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm py-3 px-4 text-gray-900">
                                    <option value="">Bulan</option>
                                    @for($i=1; $i<=12; $i++)
                                        <option value="{{ $i }}">{{ date('F', mktime(0, 0, 0, $i, 10)) }}</option>
                                    @endfor
                                </select>
                                <select wire:model.live="filterYear" class="w-1/2 bg-white text-sm border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm py-3 px-4 text-gray-900">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($galleries as $item)
                            <tr class="hover:bg-gray-50 transition-colors {{ in_array($item->id, $selectedItems) ? 'bg-green-50/50' : '' }}">
                                ...
                                <td class="px-6 py-4 text-center font-medium text-zinc-500">{{ ($galleries->currentPage() - 1) * $galleries->perPage() + $loop->iteration }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <a href="{{ url('/galeri/' . $item->slug) }}" target="_blank" class="p-1.5 text-zinc-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="Lihat">
                                    ...
                                    @if(!auth()->user()->hasRole('Mitra Media') || $item->user_id === auth()->id())
                                        <a href="{{ route('admin.gallery.edit', $item->id) }}" class="p-1.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
