                    ...
                    <tr wire:key="tag-{{ $tag->id }}" class="hover:bg-zinc-50 transition-colors {{ in_array((string)$tag->id, $selectedItems) ? 'bg-primary-50/50' : '' }}">
                        <td class="p-4 w-4" x-show="selectionMode" x-transition>
                            <div class="flex items-center">
                                ...
                                <td class="px-6 py-4 text-center font-medium text-zinc-500">{{ ($tags->currentPage() - 1) * $tags->perPage() + $loop->iteration }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if(!auth()->user()->hasRole('Mitra Media') || $tag->user_id === auth()->id())
                                <button wire:click="edit({{ $tag->id }})" class="p-1.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                ...
                                <button type="button" @click="$dispatch('open-confirm-modal', { title: 'Konfirmasi Tindakan', message: 'Apakah Anda yakin ingin menghapus tag ini?', confirmText: 'Ya, Lanjutkan', onConfirm: () => @this.delete({{ $tag->id }}) })" class="p-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
