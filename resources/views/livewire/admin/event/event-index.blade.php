            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <button @click="selectionMode = !selectionMode; if(!selectionMode) { $wire.cancelBatchDelete() }" class="inline-flex items-center px-4 py-2 bg-white border border-zinc-300 rounded-lg font-semibold text-xs text-zinc-700 uppercase tracking-widest hover:bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-primary-500 transition shadow-sm" :class="{'bg-primary-50 border-primary-300 text-primary-700': selectionMode}">
                        <svg class="w-4 h-4 mr-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                        <span x-text="selectionMode ? 'Batal Pilih' : 'Pilih Banyak'"></span>
                    </button>
                    <button wire:click="create" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 focus:bg-primary-700 active:bg-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm shrink-0">
                        <svg class="w-4 h-4 mr-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        Tambah Acara Baru
                    </button>
                </div>
            </div>
            ...
            <div x-data="{ showFilters: false }" class="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden mb-6">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-4">
                    ...
                    <div class="w-full sm:max-w-xs relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari acara..." class="bg-zinc-100 focus:bg-white text-sm border-zinc-300 focus:border-primary-500 focus:ring-primary-500 rounded-lg shadow-sm block w-full pl-9 py-3 transition-colors">
                    </div>
                ...
                <div x-show="showFilters" x-collapse>
                    <div class="p-4 bg-zinc-50 border-t border-zinc-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select wire:model.live="filterStatus" class="w-full bg-white text-sm border-zinc-300 focus:border-primary-500 focus:ring-primary-500 rounded-lg shadow-sm py-3 px-4 text-zinc-900">
                ...
                <div class="overflow-x-auto relative">
                    ...
                    <table wire:loading.class="opacity-50 pointer-events-none" wire:target="search, perPage, gotoPage, nextPage, previousPage, filterType, filterStatus, filterCategory, filterAuthor, filterMonth, filterYear, filterTag" class="w-full text-sm text-left text-zinc-600">
                        ...
                        <tbody class="divide-y divide-zinc-200">
                            ...
                            <tr class="hover:bg-zinc-50 transition-colors {{ in_array($event->id, $selectedItems) ? 'bg-primary-50/50' : '' }}">
                                <td class="p-4 w-4" x-show="selectionMode" x-transition>
                                    <div class="flex items-center">
                                        <input type="checkbox" wire:model.live="selectedItems" value="{{ $event->id }}" class="w-4 h-4 text-primary-600 bg-zinc-100 border-zinc-300 rounded focus:ring-primary-500 cursor-pointer">
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center font-medium text-zinc-500">{{ ($events->currentPage() - 1) * $events->perPage() + $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        @endforeach
                                        @if($event->issues->isEmpty() && $event->tags->isEmpty())
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <a href="{{ url('/acara/' . $event->slug) }}" target="_blank" class="p-1.5 text-zinc-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="Lihat">
                                    ...
                                    @if(!auth()->user()->hasRole('Mitra Media') || $event->user_id === auth()->id())
                                        <button wire:click="edit({{ $event->id }})" class="p-1.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                        ...
                                        <button @click="$dispatch('open-confirm-modal', {
                                                ...
                                                message: 'Data agenda acara akan dihapus secara permanen. File media sisipan teks akan dialihkan ke menu <b>Tempat Sampah</b> dan dibersihkan secara otomatis oleh sistem dalam waktu 1x24 jam. Pembersihan manual juga dapat dilakukan sewaktu-waktu melalui menu tersebut. Lanjutkan proses penghapusan?',
                            ...
                            @empty
                            <tr>
                                <td colspan="10" class="px-6 py-8 text-center text-zinc-500">Belum ada agenda acara yang ditambahkan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-40 bg-zinc-900 rounded-full shadow-2xl border border-zinc-700 p-2 pl-4 pr-2 flex justify-between items-center transition-all duration-300">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button @click="$dispatch('open-confirm-modal', {
                            ...
                            message: 'Anda akan menghapus {{ count($selectedItems) }} data acara secara permanen beserta file media sisipannya. Lanjutkan?',
    ...
    @else
        ...
        <div class="space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                ...
                <button wire:click="closeForm" class="inline-flex items-center px-4 py-2 bg-white border border-zinc-300 rounded-lg font-semibold text-xs text-zinc-700 uppercase tracking-widest hover:bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-primary-500 transition shadow-sm w-fit">
                ...
                <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <div class="w-full lg:w-80 space-y-6">
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-zinc-200 space-y-4">
                            ...
                            <div>
                                <select wire:model="is_published" class="bg-zinc-100 focus:bg-white border-zinc-300 focus:border-primary-500 focus:ring-primary-500 rounded-lg shadow-sm w-full py-3 px-4 text-zinc-900 transition-colors">
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-bold text-zinc-900" id="modal-title">
                                    Edit Label Massal (<span x-text="$wire.selectedItems.length"></span> Acara)
                                </h3>
                ...
                <div>
                    ...
                    <p class="text-sm text-zinc-600 mb-6">
                        ...
                        <template x-if="failed > 0">
                            <span>Namun <strong x-text="failed" class="text-red-600"></strong> item gagal dihapus.</span>
                        </template>
                    </p>
