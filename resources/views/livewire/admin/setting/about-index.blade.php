<div>
    ...
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-show="show" x-transition.opacity class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 rounded shadow-sm flex items-center justify-between flex items-center justify-between">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea wire:model="tujuan_quote" rows="3" class="bg-gray-100 focus:bg-white transition-colors w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 p-3" placeholder='Misal: "Sebagai ruang belajar..."'></textarea>
                ...
                <div>
                    ...
                    @foreach($tujuan_list as $index => $item)
                        <div class="flex gap-2 mb-2">
                            <textarea wire:model="tujuan_list.{{ $index }}" rows="3" class="bg-gray-100 focus:bg-white transition-colors flex-1 rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 p-3" placeholder="Masukkan poin tujuan"></textarea>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    @foreach($intensi_list as $index => $item)
                        <div class="flex gap-2 mb-2">
                            <textarea wire:model="intensi_list.{{ $index }}" rows="3" class="bg-gray-100 focus:bg-white transition-colors flex-1 rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 p-3" placeholder="Masukkan poin intensi"></textarea>
                    ...
                    @foreach($sikap_list as $index => $item)
                        <div class="flex gap-2 mb-4 p-4 border border-gray-100 rounded-lg bg-gray-50">
                            <div class="flex-1 space-y-3">
                                <div>
                                    <input type="text" wire:model="sikap_list.{{ $index }}.title" class="bg-gray-100 focus:bg-white transition-colors w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 font-medium py-3 px-4" placeholder="Judul Sikap (Misal: Independen & Objektif)">
                                    @error('sikap_list.'.$index.'.title') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <textarea wire:model="sikap_list.{{ $index }}.description" rows="3" class="bg-gray-100 focus:bg-white transition-colors w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 p-3" placeholder="Deskripsi sikap"></textarea>
