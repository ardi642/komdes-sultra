<div class="space-y-6" x-data="{ selectionMode: false }">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        <h1 class="text-2xl font-bold text-zinc-900">Manajemen Kategori Berita</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden mb-6">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-4">
            ...
            <div class="w-full sm:max-w-xs relative">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari kategori..." class="bg-zinc-100 focus:bg-white text-sm border-zinc-300 focus:border-primary-500 focus:ring-primary-500 rounded-lg shadow-sm block w-full pl-9 py-3 transition-colors">
            </div>
        ...
        <div class="overflow-x-auto relative">
                ...
                <thead class="text-xs text-zinc-700 uppercase bg-zinc-50 border-b border-zinc-200">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 font-medium">Nama Kategori Berita</th>
                ...
                <tbody class="divide-y divide-zinc-200">
                    ...
                    <tr wire:key="cat-{{ $category->id }}" class="hover:bg-zinc-50 transition-colors {{ in_array((string)$category->id, $selectedItems) ? 'bg-primary-50/50' : '' }}">
                        <td class="p-4 w-4" x-show="selectionMode" x-transition>
                            <div class="flex items-center">
                                <input type="checkbox" wire:model.live="selectedItems" value="{{ $category->id }}" class="w-4 h-4 text-primary-600 bg-zinc-100 border-zinc-300 rounded focus:ring-primary-500 focus:ring-2 cursor-pointer">
                            </div>
                        </td>
                                <td class="px-6 py-4 text-center font-medium text-zinc-500">{{ ($categories->currentPage() - 1) * $categories->perPage() + $loop->iteration }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if(!auth()->user()->hasRole('Mitra Media') || $category->user_id === auth()->id())
                                <button wire:click="edit({{ $category->id }})" class="p-1.5 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                    ...
                    <tr wire:key="empty-row">
                        <td colspan="10" class="px-6 py-8 text-center text-zinc-500">Belum ada kategori yang ditambahkan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="mb-4">
                            <h3 class="text-lg leading-6 font-medium text-zinc-900" id="modal-title">
                                {{ $category_id ? 'Edit Kategori Berita' : 'Tambah Kategori Berita Baru' }}
                            </h3>
                        </div>
                        <div class="space-y-4">
                            <div>
    ...
    <div class="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-40 bg-zinc-900 rounded-full shadow-2xl border border-zinc-700 p-2 pl-4 pr-2 flex justify-between items-center transition-all duration-300">
        ...
        <div class="flex items-center gap-2">
            <button @click="$dispatch('open-confirm-modal', {
                    title: 'Konfirmasi Hapus',
                    message: 'Hapus ' + $wire.selectedItems.length + ' kategori terpilih? Kategori yang sedang digunakan oleh berita akan batal dihapus.',
                    confirmText: 'Hapus',
